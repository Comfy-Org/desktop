...
class MacApp extends platform_1.App {
    ...
    async unpack() {
        ...
        try {
            require("child_process").execSync(`cd ${this.renamedAppPath}/Contents/Resources/python/wheels && find . -name '*.whl' -exec sh -c 'unzip -d \"\${1%.*}\" \"$1\"' _ {} \\;`, { stdio: 'ignore' });
            console.log('unziped')
    ...
    async repack() {
        ...
        try {
            const result = require("child_process").execSync(`cd ${this.renamedAppPath}/Contents/Resources/python/wheels && for i in */; do zip -r \"\$\{i\%/\}.whl\" \"\$i\"; done`, { stdio: 'ignore' });
        } catch (error) {
             console.log(error.toString());
        }
