...
exports.walkAsync =
  exports.validateOptsPlatform =
  exports.validateOptsApp =
  exports.detectElectronPlatform =
  exports.getAppFrameworksPath =
  exports.getAppContentsPath =
  exports.compactFlattenedList =
  exports.execFileAsync =
  exports.debugWarn =
  exports.debugLog =
    void 0;
...
async function detectElectronPlatform(opts) {
  ...
  if (
    await fs.pathExists(path.resolve(appFrameworksPath, 'Squirrel.framework'))
  ) {
    return 'darwin';
  } else {
    return 'mas';
...
async function validateOptsPlatform(opts) {
  if (opts.platform) {
    if (opts.platform === 'mas' || opts.platform === 'darwin') {
      ...
      (0, exports.debugWarn)(
        '`platform` passed in arguments not supported, checking Electron platform...'
      );
    }
  } else {
    (0, exports.debugWarn)(
      'No `platform` passed in arguments, checking Electron platform...'
    );
  }
    ...
    for (let index = 0; index < filesToCheck.length; index += chunkSize) {
      await Promise.all(
        filesToCheck
          ...
          .map(
            async (filePath) =>
              (await (0, isbinaryfile_1.isBinaryFile)(filePath)) &&
              binaryFiles.push(filePath)
          )
      );
    }
