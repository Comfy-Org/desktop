...
describe('validateVirtualEnvironment', () => {
  test('returns success immediately when no imports provided', async () => {
    ...
    const result = await validateVirtualEnvironment(venv, []);
  ...
  test('passes Python -c script with provided imports', async () => {
    ...
    const result = await validateVirtualEnvironment(venv, imports);
    expect(result).toEqual({ success: true });
    ...
    expect(log.info).toHaveBeenCalledWith('Virtual environment validation successful - all imports available');
  });
  ...
  test('returns missing imports when Python reports failures', async () => {
    ...
    const result = await validateVirtualEnvironment(venv, ['toml', 'uv', 'yaml']);
  ...
  test('handles invalid JSON format from Python (schema validation failure)', async () => {
    ...
    const result = await validateVirtualEnvironment(venv, ['yaml']);
    expect(result.success).toBe(false);
    expect(result.error).toMatch(/^Invalid validation output format:/);
